# Script for deploying EBMS Main Update to a CBIIT tier.
...
export REPO_URL=$NCIOCPL/ebms/tarball/Main
export WORKDIR=/tmp/ebms-Main
...
if [ -d $WORKDIR ]; then
    TIMESTAMPED=`/bin/date +"/tmp/ebms-Main-%Y%m%d%H%M%S"`
