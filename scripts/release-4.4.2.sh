# Script for deploying EBMS Release 4.4.2 ("Flathead") to a CBIIT tier.
...
export REPO_URL=$NCIOCPL/ebms/tarball/Flathead
export WORKDIR=/tmp/ebms-4.4.2
...
if [ -d $WORKDIR ]; then
    TIMESTAMPED=`/bin/date +"/tmp/ebms-4.4.2-%Y%m%d%H%M%S"`
...
$DRUSH config:import --partial --source=config/saml_patch -y || {
